...
import { createAgent, clearAgentCache } from "./create-agent";
...
export async function POST(
  ...
  try {
    // 1️. Extract user message and parameters from the request body
    const { userMessage, phoneNumber, createNewWallet } = await req.json();

    // 2. Clear agent cache if creating new wallet to get fresh context
    if (createNewWallet && phoneNumber) {
      clearAgentCache(phoneNumber);
    }

    // 3. Get the agent for the specific phone number
    const agent = await createAgent(phoneNumber, createNewWallet);

    // 4.Start streaming the agent's response
    const stream = await agent.stream(
      { messages: [{ content: userMessage, role: "user" }] }, // The new message to send to the agent
      { configurable: { thread_id: phoneNumber ? `AgentKit Discussion - ${phoneNumber}` : "AgentKit Discussion" } }, // Customizable thread ID for tracking conversations
    );

    // 5️. Process the streamed response chunks into a single message
    let agentResponse = "";
